...
exports.CAPTCHA_FLAG = 'captcha_blocked';
...
async function getFlag(key) {
    if (!_initialized)
        return false;
    return (await node_persist_1.default.getItem(key)) === true;
}
async function saveFlag(key, value) {
    if (!_initialized)
        return;
    await node_persist_1.default.setItem(key, value);
}
async function clearFlag(key) {
    if (!_initialized)
        return;
    await node_persist_1.default.removeItem(key);
}
